    ...
    public class ApplicationDbContext :DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder builder)
        {
            ...
            List<User> users = new List<User> {
                new User{ Id = Guid.NewGuid(), Email = "example@gmail.com", PasswordHash = passwordHasher.Generate("cool"), Role = "User", Username="Jake"},
                new User { Id = Guid.NewGuid(), Email = "admin@gmail.com", PasswordHash = passwordHasher.Generate("admin"), Role = "Admin", Username = "Mark" },
                new User { Id = Guid.NewGuid(), Email = "great@gmail.com", PasswordHash = passwordHasher.Generate("very"), Role = "User", Username = "Victor" },
            };

            List<Attendee> attendees = new List<Attendee>
            {
                new Attendee{ Id = Guid.NewGuid(), Name = "Peter", Surname="Parker", Email="example@gmail.com", DateOfBirth=DateTime.Parse("1960-10-10"), DateOfRegistration = DateTime.Now, SocialEventId=events[0].Id, UserId = users[0].Id },
                new Attendee{ Id = Guid.NewGuid(), Name = "Bruce", Surname="Banner", Email="great@gmail.com", DateOfBirth=DateTime.Parse("1980-01-10"), DateOfRegistration = DateTime.Now, SocialEventId=events[1].Id, UserId = users[1].Id  },
                new Attendee{ Id = Guid.NewGuid(), Name = "Clark", Surname="Kent", Email="great@gmail.com", DateOfBirth=DateTime.Parse("1990-01-10"), DateOfRegistration = DateTime.Now, SocialEventId=events[2].Id, UserId = users[2].Id  }
            };

            builder.Entity<SocialEvent>().HasData(events);
            builder.Entity<User>().HasData(users);
            builder.Entity<Attendee>().HasData(attendees);
        }

        //!!!!!!Disable comment to create a migration!!!!!!
